...
void dv_find_shortest_path (location_t start, DraculaView gv,
			int playerNo,int round,int distance[NUM_MAP_LOCATIONS]) ;
location_t dv_find_next_location (location_t start, location_t end,DraculaView gv);
location_t *dv_get_reachable (DraculaView dv, location_t start,size_t* num,bool city, bool sea);

location_t *dv_get_old_dests (DraculaView dv, size_t *n_locations, bool road, bool sea);
